...
- **Wholesale tax is per product, not per channel** (changed 2026-09-15). Wholesale was exempt by construction: `taxCalculatorForConfig` returned `NoneCalculator` for any B2B order whatever the store config said, and migration 037 recorded that as intentional — true of a catalog that is entirely bagged coffee, which WA exempts as food for home consumption. It stops being true the moment the shop invoices a cafe for a grinder or a repair, because taxability is a property of what is sold, not of who bought it. Both channels now run the same rule: `products.tax_exempt` decides per line, `customers.tax_exempt` (a reseller permit) exempts the whole order, and the flat rate applies only inside the nexus state. Every product is currently exempt, so this computes zero on every existing order and starts billing the day a taxable SKU is added — the rollout is safe by construction rather than by flag.
- **A QuickBooks invoice cannot state its own tax; it has to reference a rate** (verified against the sandbox 2026-09-15). Sending `TxnTaxDetail: {"TotalTax": 8.80}` returns 200 and comes back `TotalTax: 0` — the number is dropped and the invoice is silently short. Sending a full `TaxLine` does produce tax, but QBO recomputes it from the `TaxRate` the line references and ignores the percentage in the request: 8.8% against California's 8% rate billed 8%. So `FindOrCreateTaxCode` matches or creates a QBO rate equal to the shop's configured one and the invoice points at it, exactly as `FindOrCreateTerm` does for payment terms — match on value (not name, which bookkeepers rename), create once, cache on the client (which `Provider` rebuilds, and so re-empties, when the configured Intuit app changes). Because QBO does its own arithmetic, the created invoice's tax is read back and compared to the order's; a divergence is logged with both numbers rather than failing, since by then the invoice exists.
...
- **Wholesale tax follows the product, not the channel** (changed 2026-09-15; this bullet previously read "wholesale orders unconditionally skip tax, regardless of store config"). The blanket exemption was right for a catalog that is entirely bagged coffee and wrong the moment the shop invoices a cafe for a grinder. See *Wholesale tax is per product, not per channel* above for the full reasoning.
