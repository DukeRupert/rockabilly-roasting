...
- **The checkout page shows a subtotal only.** Shipping is negotiated offline and billed on the invoice. Sales tax, where it applies, is also added on the invoice — most wholesale coffee is exempt (it is food, and it is bought for resale), so for most accounts the invoice total is the subtotal plus shipping. Taxable items such as equipment are the exception.
