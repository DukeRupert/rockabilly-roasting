...
| 2026-07-21 | **Pilot delta + Batch 3 migrated to PRODUCTION** | Pre-cutover delta check found 6 pilot orders placed post-Jul-7 (imported, $1786). Added no-group→2026 rule + Wholesale 2024 list (MOCHA); fixed Tailwinds 12O (1250→1100, convention Hiri-12O←OS-1LB). Batch 3 = 12 (MOCHA→2024, Tailwind→Tailwinds, no-group→2026, Kool Beanz suspended, Baseling pending): 147 orders, 326 lines, 0 warnings; 10/10 approved welcome emails sent. Prod 210→222 customers, 1196 OS- orders. **Parked: Bunker (white-label); excluded: Kagen's, Firefly (tests).** Final delta clean — only test accounts left un-migrated. |
