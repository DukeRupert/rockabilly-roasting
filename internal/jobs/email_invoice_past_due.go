...
// Work sends the past-due reminder email for the stage in the job args.
func (w *InvoicePastDueEmailWorker) Work(ctx context.Context, job *river.Job[EmailInvoicePastDueArgs]) error {
	return w.orders.SendInvoicePastDueEmail(ctx, w.pool, job.Args.OrderID, job.Args.CustomerID, job.Args.Stage, job.Args.DueDate)
}
