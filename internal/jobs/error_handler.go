...
// logWorkerFailure. The rule for the Error logs that remain is stated rather
// than listed, because a list of call sites rots: they are all errors that are
// *swallowed* — logged inside a loop or a best-effort side path and never
// returned — so nothing else will ever report them.
//
