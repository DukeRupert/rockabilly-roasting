...
func NewShippoTrackingUpdateWorker(fulfillmentSvc *app.FulfillmentService, pool *pgxpool.Pool) *ShippoTrackingUpdateWorker {
	return &ShippoTrackingUpdateWorker{fulfillmentSvc: fulfillmentSvc, pool: pool}
}

// Work applies the tracking update inside a single transaction.
func (w *ShippoTrackingUpdateWorker) Work(ctx context.Context, job *river.Job[ShippoTrackingUpdateArgs]) error {
	...
	err := store.Tx(ctx, w.pool, func(tx pgx.Tx) error {
		_, applyErr := w.fulfillmentSvc.ApplyTrackingStatus(ctx, tx, args.TrackingNumber, args.Status)
		return applyErr
	})
