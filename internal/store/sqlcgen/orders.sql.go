...
const switchOrderToPickup = `-- name: SwitchOrderToPickup :one
UPDATE orders
...
func (q *Queries) SwitchOrderToPickup(ctx context.Context, id uuid.UUID) (Order, error) {
	row := q.db.QueryRow(ctx, switchOrderToPickup, id)
	var i Order
	err := row.Scan(
		&i.ID,
		&i.Number,
